...
local_means_brack <- function(sample,n.grid){
  grid.b<-seq(0,1,1/n.grid)
  grid <- grid.b[-n.grid-1]+(grid.b[2]-grid.b[1])/2
  #bracketing
  for (j in 1:length(sample$x)){
...
cp_grid <- function(cp,n,n.grid){
  ...
  for (j in 1:length(cp)){
    i.low <- min(which(cp[j]/n<=grid.b))-1
    cp.grid <- c(cp.grid,i.low)
  }
  return(list(cp.grid=cp.grid,
         grid=grid,
         grid.b=grid.b))
}


solo_spike_slab_brack<-function(sample,tau2,tau2.spike,tau2.slab,q,sigma2,n.grid){
  n<-length(sample$x)
  #define grid
  grid.b<-seq(0,1,1/n.grid)
  grid <- grid.b[-n.grid-1]+(grid.b[2]-grid.b[1])/2
  ...
  lower<-tau2*n1[n.grid]^2/(tau2*n1[n.grid]+sigma2) #this is going down
  lower.local.means<-yy[n.grid] #this is going down
  for (j1 in (n.grid-1):1){
    new.lower<-tau2*(sum(n1[j1:n.grid])-sum(lower))^2/(tau2*(sum(n1[j1:n.grid])-(sum(lower)))+sigma2)
    new.lower.mean<-(sum(yy[j1:n.grid])-sum(lower.local.means*lower))/(sum(n1[j1:n.grid])-sum(lower))
  ...
  GAM<-matrix(NA,nrow=n.grid,ncol=n.grid)
  M<-matrix(NA,nrow=n.grid,ncol=n.grid)
  Y<-matrix(NA,nrow=n.grid,ncol=n.grid)
  
  sum.y<-rev(cumsum(rev(yy)))
  ...
  GAM[2:n.grid,2]<-1-(sum.inv[2] - marg.weight[2:n.grid])*M[2:n.grid,1]
  Y[2:n.grid,2]<-sum.y[2]-marg.mean[2:n.grid]-(sum.inv[2] - marg.weight[2:n.grid])*M[2:n.grid,1]*GAM[2:n.grid,1]*Y[2:n.grid,1]
  #weight
  w.spike<-sqrt(tau2.spike^(-1)/(lower[1]+tau2.spike^(-1)))*exp(1/(2*sigma2)*(sum(yy)-sum(lower.local.means[2:n.grid]*lower[2:n.grid]))^2/(lower[1]+tau2.spike^(-1)))#maybe wrong but it does not matter
  w.slab<-sqrt(tau2.slab^(-1)/(lower[1]+tau2.slab^(-1)))*exp(1/(2*sigma2)*(sum(yy)-sum(lower.local.means[2:n.grid]*lower[2:n.grid]))^2/(lower[1]+tau2.slab^(-1)))#maybe wrong but it does not matter
  ...
  for (i in 3:(n.grid-1)){
    #param
    M[(i-1):n.grid,(i-1)]<-1/((sum.inv[(i-1)] - marg.weight[(i-1):n.grid])*GAM[(i-1):n.grid,(i-1)]+sigma2*tau2^(-1))
    GAM[i:n.grid,i]<-1-(sum.inv[i] - marg.weight[i:n.grid])*rowSums(M[i:n.grid,1:(i-1)]*GAM[i:n.grid,1:(i-1)]^2)
    Y[i:n.grid,i]<-sum.y[i]-marg.mean[i:n.grid]-(sum.inv[i] - marg.weight[i:n.grid])*rowSums(M[i:n.grid,1:(i-1)]*GAM[i:n.grid,1:(i-1)]*Y[i:n.grid,1:(i-1)])
    #weight
  ...
  #param
  M[(n.grid-1):n.grid,(n.grid-1)]<-1/((sum.inv[(n.grid-1)] - marg.weight[(n.grid-1):n.grid])*GAM[(n.grid-1):n.grid,(n.grid-1)]+sigma2*tau2^(-1))
  GAM[n.grid,n.grid]<-1-(sum.inv[n.grid] - marg.weight[n.grid])*sum(M[n.grid,1:(n.grid-1)]*GAM[n.grid,1:(n.grid-1)]^2)
  Y[n.grid,n.grid]<-sum.y[i]-marg.mean[n.grid]-(sum.inv[n.grid] - marg.weight[n.grid])*sum(M[n.grid,1:(n.grid-1)]*GAM[n.grid,1:(n.grid-1)]*Y[n.grid,1:(n.grid-1)])
  #weight
  w.spike<-c(w.spike,sqrt(tau2.spike^(-1)/((sum.inv[n.grid] - marg.weight[n.grid])*GAM[n.grid-1,n.grid-1]+tau2.spike^(-1)))*exp(1/(2*sigma2)*Y[n.grid,n.grid]^2/((sum.inv[n.grid] - marg.weight[n.grid])*GAM[n.grid-1,n.grid-1]+tau2.spike^(-1))))
  w.slab<-c(w.slab,sqrt(tau2.slab^(-1)/((sum.inv[n.grid] - marg.weight[n.grid])*GAM[n.grid-1,n.grid-1]+tau2.slab^(-1)))*exp(1/(2*sigma2)*Y[n.grid,n.grid]^2/((sum.inv[n.grid] - marg.weight[n.grid])*GAM[n.grid-1,n.grid-1]+tau2.slab^(-1))))
