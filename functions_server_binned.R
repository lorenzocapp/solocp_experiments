...
X_mat_brack <- function(n.grid,sample,n){
  ...
  n1<-rep(0,n.grid)
  n1[idn]<-n01
  Xl<-matrix(1,nrow =n.grid,ncol=n.grid)
  Xl[upper.tri(Xl)] <- 0
  X <- matrix(rep(Xl[1,],n1[1]),ncol=n.grid,byrow=T)
  for (i in 2:n.grid){
    add <- matrix(rep(Xl[i,],n1[i]),ncol=n.grid,byrow=T)
    X <- rbind(X,add)
  }
  return(X)
}
